drop table if exists `apigatewaydb`.`security_user`;
drop table if exists `apigatewaydb`.`role`;
drop table if exists `apigatewaydb`.`authority`;
drop table if exists `apigatewaydb`.`user_role`;
drop table if exists `apigatewaydb`.`role_authority`;
CREATE TABLE IF NOT EXISTS `apigatewaydb`.`security_user` (
...
CREATE TABLE IF NOT EXISTS `apigatewaydb`.`role` (
...
CREATE TABLE IF NOT EXISTS `apigatewaydb`.`authority` (
...
CREATE TABLE IF NOT EXISTS `apigatewaydb`.`user_role` (
    ...
    CONSTRAINT `user_id`
    FOREIGN KEY (`user_id`)
    REFERENCES `apigatewaydb`.`security_user` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
    CONSTRAINT `role_id`
    FOREIGN KEY (`role_id`)
    REFERENCES `apigatewaydb`.`role` (`role_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
    ENGINE = InnoDB
    COMMENT = 'The mapping table, to map the users with respective roles.';

CREATE TABLE IF NOT EXISTS `apigatewaydb`.`role_authority` (
    ...
    REFERENCES `apigatewaydb`.`authority` (`authority_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
    ENGINE = InnoDB
